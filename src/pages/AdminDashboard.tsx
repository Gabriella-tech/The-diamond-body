      ...
      <Container className="py-10">
        <div className="bg-[#222] rounded-3xl p-6 sm:p-8 text-white mb-6 flex flex-col sm:flex-row justify-between sm:items-center gap-3">
          ...
          <div className="flex gap-2 items-center">
            ...
            <button
              onClick={() => { setUser(null); navigate("/"); toast({ type: "info", message: "Signed out" }); }}
          ...
          <main className="space-y-6">
            ...
            {tab === "settings" && (
              <div className="bg-white rounded-2xl border border-gray-100 p-6 max-w-2xl">
                ...
                <Button className="mt-6" onClick={() => toast({ type: "success", message: "Settings saved" })}>Save Settings</Button>
              </div>
            )}
          </main>
        </div>
            ...
            {stations.map((s) => (
              <tr key={s.id} className="border-b border-gray-50 hover:bg-gray-50">
                ...
                <td className="py-3 pr-3">
                  ...
                  <div className="text-xs text-gray-500">{s.hours}</div>
                </td>
...
function PickupStationForm({ station, onClose, onSave }: {
  ...
  const [form, setForm] = useState<Partial<PickupStation>>(station || {
    name: "", address: "", city: "", state: "", hours: "Mon–Sat, 9am–6pm", status: "active" as PickupStationStatus,
  });
