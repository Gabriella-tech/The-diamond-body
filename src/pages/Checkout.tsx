...
export function Checkout() {
  ...
  const placeOrder = (e: React.FormEvent) => {
    ...
    if (deliveryMethod === "Pickup Station") {
      ...
      if (!pickupStation) {
        toast({ type: "error", message: "Please select a pickup station." });
        return;
      }
    }
    ...
    const order: Order = {
      ...
      paymentStatus: method === "Paystack" ? "Paid" : proofFile ? "Awaiting Verification" : "Unpaid",
        ...
        <div className="space-y-8">
          ...
          <div className="bg-white border border-gray-100 rounded-2xl p-6">
            ...
            <div className="space-y-3">
              ...
              <label className={`flex items-start gap-3 p-4 rounded-xl border-2 cursor-pointer transition ${method === "Bank Transfer" ? "border-[#4A0E16] bg-[#4A0E16]/5" : "border-gray-200"}`}>
                <input type="radio" name="pm" checked={method === "Bank Transfer"} onChange={() => setMethod("Bank Transfer")} className="mt-1 accent-[#4A0E16]"/>
                <div className="flex-1">
                  <div className="font-semibold">Direct Bank Transfer</div>
                  <div className="text-xs text-gray-500">Transfer to our account and upload proof</div>
                </div>
              </label>
            </div>
            ...
            {method === "Bank Transfer" && (
              <div className="mt-4 p-4 bg-[#F5F5F5] rounded-xl space-y-3 text-sm">
                ...
                <p className="text-xs text-gray-600">{BANK_DETAILS.reference}</p>
